...
**チェックポイント（多言語対応版・フランス語d=0/d=1フィックス）**: コミット`cb54dc2`時点。2026-08-20版チェックポイントを土台に、`d=1`時のlg強制無視の撤廃、フランス語表示の既存バグ修正（`msg_H0`系未設定・誤訳・タイプミス）、方向ゲージラベル・デモバナー・二重タブガードのフランス語対応を実施（これらは`p=0`/`p=1`共通のロジックのため両方に効く）。加えて`p=0`向けに、文字幅自動縮小（`fitDispFont`）・限界到達メッセージの冗長表示簡略化を実施（`p=1`の△パネルUI側は今回このp=0固有の対応は未反映）。d=1（Playwright）・d=0（実機、スマホSafari）双方で日本語・フランス語の表示崩れ（undefined・2行折り返し等）がないことを確認済み。3言語目以降の追加は現時点では未着手（2言語での安定化が優先）。
...
### 多言語対応の原則（フランス語対応を機に追加）

`lg`（言語）は`DEMO_MODE`/`PANEL_MODE`とは別の第3の軸として扱う。言語によって文字列の長さが大きく異なる（フランス語は日本語より長くなりがち）ため、表示崩れ対策は次の方針で行うこと。

- **言語ごとに個別の表示調整コード（if/elseで言語分岐したレイアウト処理等）を書かない**。代わりに、実際の文字幅を測って収まらなければ自動縮小する等の**言語非依存・測定ベースの汎用ロジック**（`fitDispFont`・パネルタイトルの自動縮小と同じ考え方）で対応する。これにより将来言語が増えても同じ仕組みがそのまま使える
- 上記の方針により、**日本語（既定言語）の見た目は通常変化しない**はずである（日本語文字列は元々短く、縮小ロジックが発動しないケースが大半のため）。多言語対応の変更を行う際は、日本語表示に意図しない影響が出ていないか必ず確認すること
...
- **フランス語体験（`p=0`、d=0/d=1とも一旦フィックス済み）**: `d=1`時にlgを無視し常に日本語固定だった仕様を撤廃、既存のフランス語バグ（高さ0=いちばんした関連の文言が丸ごと未設定でundefinedが出ていた、btn_H0の誤訳、msg_Lのタイプミス）を修正、方向ゲージのSVGラベル（ひだり/みぎ）・デモバナー・二重タブガードのブロック画面もフランス語対応、`p=0`の上部メッセージが長い文字列で2行に折り返る問題（文字幅自動縮小・限界到達メッセージの冗長表示簡略化）も修正した。d=1はPlaywright、d=0はスマホ実機（Safari）で日本語・フランス語とも表示崩れがないことを確認済み。**`p=1`（△パネルUI）は今回のp=0固有の折り返し対策（文字幅自動縮小等）は未反映**（共通ロジックの修正・バグ修正分は`p=1`にも効いている）。3言語目以降の追加は現時点では未着手
