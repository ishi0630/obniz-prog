...
| `lg` | 表示言語(J=日本語 / それ以外=フランス語)。大文字小文字区別なし | `J`（`d=1`時も含めlgをそのまま反映する。以前は`d=1`時に常にJ固定だったが、実機なしでもフランス語を確認・デモできるよう変更した） |
...
- **フランス語体験（次期メジャーアップデート、対応中）**: `d=1`時にlgを無視し常に日本語固定だった仕様を変更し、`d=0`/`d=1`どちらでも`lg=F`でフランス語表示できるようにした。あわせて既存のフランス語バグ（高さ0=いちばんした関連の文言が丸ごと未設定でundefinedが出ていた、btn_H0の誤訳、msg_Lのタイプミス）を修正し、方向ゲージのSVGラベル（ひだり/みぎ）・デモバナー・二重タブガードのブロック画面もフランス語対応した。対象は`p=0`（Playwrightでは`p=1`経由でロジックを確認、`p=0`実物は外部CDN依存のためこの環境では未確認）。フランス語は日本語より文字列が長くなる場合があるため、スマホ向けレイアウト（文字サイズ縮小・折り返し等）でのフランス語表示は依然未確認・要調整
- `main`ブランチへのマージ（現状`claude/online-boccia-robot-lamp-grtof1`ブランチのみで運用中）
- たかさ・方向のゲージ状イラスト表示は、`RAMP-ILLUSTRATION-POC.html`という別ファイルで開発していたが、`RAMP-DEMO.html`へ統合した（その後`RAMP-ILLUSTRATION-POC.html`もデモ用として使いたいとの要望があり一旦`RAMP-DEMO.html`と同一内容で復元したが、`d=0`実機動作の確認がまだのため、改めて「一旦フィックスした」時点（コミット`f28d58c`、機能コードは現行`RAMP-DEMO.html`と同一・ヘッダーコメントのみ異なる）に固定し直した）。**`RAMP-ILLUSTRATION-POC.html`は`d=0`確認が済むまで`RAMP-DEMO.html`とは同期させない**（`RAMP-DEMO.html`側で今後行う実機向けの変更は、確認が取れるまでこのファイルには反映しない）。コードレビュー上は`d=0`実機モードでも(--ramp-colorの事前設定等)問題ないはずだが、**実機・実ネットワーク環境での動作確認は依然として未実施**（上記`d=0`確認タスクと同一）。詳細な経緯は`docs/ramp-illustration-poc.md`参照
