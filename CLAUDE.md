...
**次期メジャーアップデート（対応中）：多言語対応がメインテーマ**。2026-08-20版チェックポイントを土台に、`d=1`時のlg強制無視の撤廃、フランス語表示の既存バグ修正（`msg_H0`系未設定・誤訳等）、`p=0`向けの文字幅自動縮小・限界到達メッセージ簡略化を実施済み。詳細は「多言語対応の原則」節および「既知の残タスク」参照。
